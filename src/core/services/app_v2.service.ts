...
export const createApp = async (userContext: IUserContext, app: IApp) => {
  try {
    ...
    const newCome = {
      ...
      created_from: app.created_from ? app.created_from : 'imbrace',
    };
...
export const getApps = async (
  ...
  try {
    ...
    const apps = await appRepository.search(org_id, {
...
const _deleteAIApp = async (app: IApp) => {
  try {
    ...
    // delete app
    await appRepository.delete(app._id || app.id);
    return {
...
export const getUsedChannels = async (
  ...
  try {
    ...
    let apps = await appRepository.findByChannelIds(ids, userContext.org_id);
    if (productId) {
      apps = apps.filter((app: any) => app.product_id === productId);
    }
    ...
    ids.forEach((id) => {
      const app = apps.find((app: any) => app.channel?.id === id);
      if (!app) {
